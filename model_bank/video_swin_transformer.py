...
class InputProj(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            ...
            x = x.flatten(2).transpose(1, 2)
            x = self.norm(x)
            x = x.transpose(1, 2).view(-1, self.out_channel, D, H, W)
        return x
...
class eca_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.avg_pool(rearrange(x, ' b d h w c -> b c d h w '))
...
class LeFF(nn.Module):
    def __init__(self, dim=32, hidden_dim=128, act_layer=nn.GELU, drop=0., use_eca=False):
        ...
        self.dwconv = nn.Sequential(
            nn.Conv3d(hidden_dim, hidden_dim, groups=hidden_dim, kernel_size=3, stride=1, padding=1),
            act_layer()
        )
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(2, 7, 7), shift_size=(0, 0, 0),
        ...
        if modulator:
            self.modulator = nn.Embedding(window_size[0] * window_size[1] * window_size[2], dim)
        else:
            self.modulator = None
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(wmx_windows, mask=attn_mask)  # B*nW, Wd*Wh*Ww, C
        # merge windows
...
class Downsample(nn.Module):
    def __init__(self, dim, norm_layer=None):
        ...
        self.conv = nn.Sequential(
            nn.Conv3d(dim, dim*2, kernel_size=4, stride=2, padding=1),
        )
...
class PatchExpand3D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = rearrange(x, 'b d h w (p1 p2 c)-> b d (h p1) (w p2) c', p1=2, p2=2, c=C//2)
        x = self.norm(x)
        x = rearrange(x, 'b d h w c-> b c d h w')
...
# 可用卷积替代
class FinalPatchExpand3D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = rearrange(x, 'b d h w (p1 p2 p3 c)-> b (d p1) (h p2) (w p3) c',
                      p1=self.patch_size[0], p2=self.patch_size[1], p3=self.patch_size[2], c=C//self.dim_scale)
        # x = self.norm(x)
        x = rearrange(x, 'b d h w c-> b c d h w')
...
class SwinTransformer3D_unet(nn.Module):
    ...
    def __init__(self, pretrained=None, pretrained2d=False,
                 patch_size=(2, 4, 4), in_chans=1, embed_dim=32,
                 depths=[2, 2, 6, 2], num_heads=[2, 4, 8, 8],
                 window_size=(4, 8, 8), mlp_ratio=4.,
        ...
        for i_layer in range(self.num_layers):
            ...
            conv = nn.Conv3d(int(embed_dim * 2 ** (self.num_layers - i_layer)),
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformer3D_unet, self).train(mode)
        self._freeze_stages()
        self.init_weights()


if __name__ == "__main__":
    # embed_dim = reduce((lambda x, y: x * y), patch_size)
    model = SwinTransformer3D_unet(in_chans=1, embed_dim=32)

    dummy_x = torch.rand(1, 1, 32, 240, 240)
