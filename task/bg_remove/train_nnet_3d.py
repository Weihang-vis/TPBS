...
from model_bank.network import Network_3D_Nnet
...
parser.add_argument('--log_folder', type=str, default='/home/wwh/code/DLT-main/log/bg_remove/nnet_3d', help="output directory")
...
parser.add_argument('--train_datasets_size', type=int, default=6000, help='datasets size for training')

parser.add_argument('--checkpoint', type=str, default='/home/wwh/code/DLT-main/log/bg_remove/nnet_3d/weights/0010_of_0100_checkpoint.pth')
...
'''<<<<<<<<<<<<<<<<<<< loss function >>>>>>>>>>>>>>>>>>>>'''
loss_fn = Lloss(0.5, mu=0.1, sigma=0.9)

'''<<<<<<<<<<<<<<<<<<< network >>>>>>>>>>>>>>>>>>>>'''
network = Network_3D_Nnet(in_channels=1, out_channels=1, f_maps=64, final_sigmoid=True)
