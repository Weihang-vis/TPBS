...
class HyperParamSearch(nn.Module):
    ...
    def objective(self, trial: Trial, callback_fn, monitor):
        ...
        history = custom_model.fit(train_loader,  val_loader,
                                   epochs=epoch,
                                   ckpt_path=None,
    ...
    def fit(self, study_name, callback_fn=None, monitor="val_loss", direction="minimize", n_trials=100, n_jobs=1, pruner=None):
        ...
        self.study.optimize(objective_fn, n_trials=n_trials, n_jobs=n_jobs)
    ...
    def get_total_trial(self):
        ...
        n = self.network.n_trials * self.loss_fn.n_trials * self.optimizer.n_trials * self.lr_scheduler.n_trials * \
            len(self.epoch_space) * len(self.seed_space)
        return n
